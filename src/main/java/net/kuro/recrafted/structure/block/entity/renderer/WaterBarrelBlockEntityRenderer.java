package net.kuro.recrafted.structure.block.entity.renderer;
...
public class WaterBarrelBlockEntityRenderer implements BlockEntityRenderer<WaterBarrelBlockEntity>
{
    private static final float[] FLUID_HEIGHT = { 0, 0.4375f, 0.6875f, 0.9375f };
    ...
    public WaterBarrelBlockEntityRenderer(BlockEntityRendererFactory.Context ignored) {

    }

    @Override
    public void render(WaterBarrelBlockEntity entity, float partialTick, MatrixStack poseStack, VertexConsumerProvider buffer, int packedLight, int packedOverlay) {
        int liquidLevel = entity.getCachedState().get(LeveledBarrelBlock.LEVEL);
        if (liquidLevel == 0)
            return;
